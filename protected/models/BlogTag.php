...
class BlogTag extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
			'posts' => array(self::MANY_MANY, 'BlogPost', 'blog_post_tag(tag_id, post_id)'),
		);
	}
    ...
    public static function getIdsFromNames($tagNames)
    {
    	$ids = array();
    	foreach (array_unique($tagNames) as $tagName) {
    		$tag =BlogTag::model()->find('name = :name', array(':name' => $tagName));
    		if (!$tag) {
    			$tag = new BlogTag();
    			$tag->name = $tagName;
    			if ($tag->save()) {
