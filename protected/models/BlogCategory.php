...
class BlogCategory extends CActiveRecord
{
    ...
    public function relations()
    {
        ...
        return array(
            'posts' => array(self::MANY_MANY, 'BlogPost', 'blog_post_category(category_id, post_id)'),
        );
    }
    ...
    public function beforeDelete()
    {
        $posts = $this->posts;
        if ($posts) {
            $ids = CHtml::listData($posts, 'id', 'id');
            $sin = implode(', ', $ids);
            BlogPost::model()->updateAll(
                array(
                    'status' => BlogPost::POST_STATUS_DRAFTED,
