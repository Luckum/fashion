...
class Php2Xml extends Common {
    ...
    public function __construct($classPrefix, $phpClass = null) {
        ...
        if (class_exists('\Zend_Registry', false)) { // this does not work due to PHP bug @see http://bugs.php.net/bug.php?id=46813
	...
	print_r($docs);
        ...
        if ($docs['xmlNamespace'] != '') {
            ...
            if (is_object($this->root)) { // root initialized
                ...
                $code = $this->getNsCode($docs['xmlNamespace'], false);
		print_r($code);
		print_r($code);
